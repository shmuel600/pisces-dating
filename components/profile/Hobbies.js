import HobbiesIcon from '@mui/icons-material/TagFacesRounded';
    ...
    return (
        <>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <Box sx={{ width: 320, minHeight: 120, pl: 3 }}>
                    <Autocomplete
                        multiple
                        id="tags-filled"
                        options={hobbiesList.map((option) => option.title)}
                        ...
                        renderTags={(value, getTagProps) =>
                            value.map((option, index) => (
                                <Chip key={option.title} label={option} {...getTagProps({ index })} />
                            ))
                        }
                        renderInput={(params) => (
                            <TextField
                                {...params}
                                variant="standard"
                                placeholder={valueLength === 0 ? 'Choose up to 6 hobbies...' : ''}
                            />
                        )}
                    />
                </Box>

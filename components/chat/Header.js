...
export default function Header({ user, isMobile }) {
    ...
    const [matchedUserProfileImage, setMatchedUserProfileImage] = React.useState(userDefault);
    React.useEffect(() => {
        fetchMatchedUser();
    ...
    const fetchMatchedUser = async () => {
        ...
        setMatchedUserProfileImage(fetchMatch.profileImage);
    }
    ...
    return (
        ...
        >
            ...
            <Image className={styles.profileImage} src={matchedUserProfileImage} alt='profile image' width={30} height={30} sx={{ borderRadius: '50%' }} />
        </div>
    )
}